public class EmployeeBook {
    private final Employee[] empls = new Employee[10];
    ...
    public void minSalary() {
        ...
        for (Employee value : empls) {
            ...
            //Сначала нахожу сам минимум, а в следующем цикле ищу всех работников, которые получают столько
        }
        for (Employee value : empls) {
    ...
    public String avgSalary() {
        ...
        double avgSalary = salarySum() * 1.0 / counter;
    ...
    public void minSalaryByUnit(int unit) {
        int min = Integer.MAX_VALUE;
        for (Employee value : empls) {
            if (value != null && value.getUnit() == unit && min > value.getSalary()) {
                min = value.getSalary();
            }
        }
        //аналогично с методом minSalary, все же я не знаю как в одном цикле найти всех, кто получает определенную сумму
        for (Employee value : empls) {
            if (value != null && value.getUnit() == unit && min == value.getSalary()) {
    ...
    public void maxSalaryByUnit(int unit) {
        int max = Integer.MIN_VALUE;
        for (Employee value : empls) {
            if (value != null && value.getUnit() == unit && max < value.getSalary()) {
                max = value.getSalary();
            }
        }
        for (Employee value : empls) {
    ...
    public int salarySumByUnit(int unit) {
        int sum = 0;
        for (Employee value : empls) {
            if (value != null && value.getUnit() == unit) {
                sum += value.getSalary();
            }
        }
        return sum;
    ...
    public void salaryLessThan(int threshold) {
        for (Employee value : empls) {
            if (value != null && value.getSalary() < threshold) {
    ...
    public void removeEmployee(String name) {
        for (int i = 0; i < empls.length; i++) {
            if (empls[i] != null && name.equals(empls[i].getName())) {
                empls[i] = null;
                break;
            }
        }
